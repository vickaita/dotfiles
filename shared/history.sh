...
if [ -n "$ZSH_VERSION" ]; then
    ...
    # History option documentation from https://zsh.sourceforge.io/Doc/Release/Options.html
    ...
    # this option is set, words read in from a history file are divided up in a
    ...
    # this option is unset, we instead truncate the old history file and write
    # out the new version in-place. If one of the history-appending options is
    # enabled, this option only has an effect when the enlarged history file
    # needs to be re-written to trim it down to size. Disable this only if you
    # have special needs, as doing so makes it possible to lose history entries
    # if zsh gets interrupted during the save.
    #
    # When writing out a copy of the history file, zsh preserves the old file’s
    # permissions and group information, but will refuse to write out a new
